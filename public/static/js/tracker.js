...
$(document).ready(function () {
	...
	if (!trackerCurrentPage.includes('/login') && !trackerCurrentPage.includes('/forgot-password') && !trackerCurrentPage.includes('/change-password')) {
		...
		$('#admin-reports-form').on('submit', function (e) {
			...
			var start = moment($('#report-start').val()).startOf('day').format('YYYY-MM-DD HH:mm:ss');
			var end = moment($('#report-end').val()).endOf('day').format('YYYY-MM-DD HH:mm:ss');
			var offset = moment().format('Z');
			var queryString = 'user_id=' + user + '&office_id=' + office + '&report_type=' + type + '&start_date=' + start + '&end_date=' + end + '&utc_offset=' + offset;

			var formData = preparePostRequest(queryString);
			var op = '';
