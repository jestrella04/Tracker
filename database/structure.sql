...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`jon`@`localhost`*/ /*!50003 TRIGGER `Tracker`.`session_BEFORE_UPDATE` BEFORE UPDATE ON `session` FOR EACH ROW
BEGIN
	SET NEW.`date_last_activity` = NOW();
END */;;
DELIMITER ;
...
BEGIN
	DELETE FROM `session` WHERE `id_status` = 9 AND `date_last_activity` < DATE_SUB(NOW(), INTERVAL 15 MINUTE);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN	
	...
	WHERE `id_status` <> 9 AND `date_last_activity` < DATE_SUB(NOW(), INTERVAL 10 MINUTE);
...
-- Dump completed on 2018-04-19 18:01:58
